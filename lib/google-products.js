 ...
 * Rules:
 *   "Aprobado"          → include, not sold
 *   "Vendido" + fecha   → include as sold if within 48 h; otherwise hide
 *   "Vendido" no fecha  → include as sold (admin hasn't set date yet)
 *   anything else / empty → hidden (Pendiente by default)
 */
function getProductPublishState(rawObj) {
  const status = normalizeKey(getValue(rawObj, STATUS_COLUMNS));

  if (status === "aprobado") return { include: true, sold: false };
  ...
  return { include: false, sold: false }; // Pendiente / Rechazado / Eliminado / vacío
}
