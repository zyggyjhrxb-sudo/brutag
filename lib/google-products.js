...
// Column the admin fills with a date when marking a product sold (for 48h auto-hide)
const FECHA_VENDIDO_COLS = ["Fecha vendido", "Fecha de venta", "Vendido el", "FechaVendido"];
...
/** TRUE/FALSE checkbox → real boolean */
...
/**
 * Reads the three boolean columns from the Sheet and returns { include, sold }.
 * "No Publicado" = TRUE → hidden (wins over everything)
 * "Vendido"      = TRUE → visible with badge; auto-hides 48h if "Fecha vendido" filled
 * "Publicado"    = TRUE → visible normally
 * All FALSE/empty → hidden (new Tally submissions default to hidden)
 */
function getProductPublishState(rawObj) {
  if (isTruthy(getValue(rawObj, ["No Publicado", "No publicado"]))) {
    return { include: false, sold: false };
  }

  if (isTruthy(getValue(rawObj, ["Vendido"]))) {
    const fechaStr = getValue(rawObj, FECHA_VENDIDO_COLS);
    if (fechaStr) {
      const fecha = new Date(String(fechaStr).trim());
      if (!isNaN(fecha.getTime()) && Date.now() - fecha.getTime() > VENDIDO_WINDOW_MS) {
  ...
  if (isTruthy(getValue(rawObj, ["Publicado", "Publicar"]))) {
    return { include: true, sold: false };
  }
...
function normalizeSheetProduct(product, rowIndex, { sold = false } = {}) {
  ...
  return {
    ...
    // "Estado del producto" is the physical condition from the Tally form
